  ...
  const register = async () => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/register/admin/checked`,
        formData,
      );
  ...
  return (
    <>
      <div className="d-flex flex-column align-items-center justify-content-center w-100">
        <h3>Admin Register Step 2</h3>
        <p className="mb-0 mt-3">Admin Username</p>
        ...
        <p className="mb-0 mt-3">Admin Password</p>
        <Form.Group className="mt-1 text-light w-25 mx-5">
          <Form.Control
            className="w-100 btn btn-dark align-items-start text-start"
            type="password"
        ...
        {error && (
          <div className="d-flex gap-2">
            <p>
              Allow registering from server cache/admin/...json file. If you
              suspect this is an error more info is at console.
            </p>
            <Link to="/help/admin-register">Help</Link>
          </div>
        )}
        {message == "User created successfully" ? (
          <div className="d-flex gap-2 alert alert-success" role="alert">
            <p className="mb-0">Admin user created</p>
